    ...
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      {contacts.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h2 className="text-lg font-medium text-gray-400 mb-2">{t('contacts.noContacts')}</h2>
          <p className="text-sm text-gray-600">{t('contacts.noContactsDesc')}</p>
        </div>
      ) : (
        <div className="space-y-2">
          {contacts.map(c => (
            <div key={c.id} className="flex items-center justify-between p-3 md:p-4 bg-gray-900 border border-gray-800 rounded-xl">
              <div className="min-w-0">
                <div className="font-medium text-sm md:text-base truncate">{c.name}</div>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-xs text-gray-500 font-mono">{shortenAddress(c.address)}</span>
                  <button
                ...
                {c.chain && <span className="text-xs text-gray-600">{c.chain}</span>}
              </div>
              <button
