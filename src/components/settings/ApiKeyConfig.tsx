...
export function ApiKeyConfig() {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
      ...
      <p className="text-sm text-gray-500 mb-5">{t('settings.aiConfigDesc')}</p>

      <div className="space-y-4">
        <div>
          <label className="block text-sm text-gray-400 mb-1">{t('settings.aiProvider')}</label>
          <div className="flex gap-2">
            <button
              onClick={() => setLocalProvider('openai')}
              className={`flex-1 py-2 text-sm rounded-lg transition-colors ${localProvider === 'openai' ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400'}`}
            >OpenAI</button>
            <button
              onClick={() => setLocalProvider('custom')}
              className={`flex-1 py-2 text-sm rounded-lg transition-colors ${localProvider === 'custom' ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400'}`}
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:border-brand-500"
              placeholder="sk-..."
            />
        ...
        <button
          onClick={handleSave}
          className="flex items-center gap-2 px-4 py-2.5 bg-brand-600 hover:bg-brand-700 rounded-lg text-sm font-medium transition-colors"
        >
        ...
        {saved && (
          <p className="text-sm text-green-400">{t('settings.configSaved')}</p>
        )}
      </div>
    </div>
  );
